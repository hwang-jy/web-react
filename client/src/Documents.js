...
function UpdateForm(props){
  const [title, setTitle] = useState(props.docs.title);
  const [desc, setDesc] = useState(props.docs.desc);
  ...
  return(
    <>
      ...
      <form onSubmit={event => {
        event.preventDefault();
        props.onSubmit(props.docs.id, title, desc);
        }}>
      <p>
...
function App() {
  ...
  if(mode === "WELCOME"){
    ...
    content = (
    <div>
      <UpdateForm docs={docs[id]} onSubmit={(_id, _title, _desc) => {
        const updateDocs = {id:id, title:_title, desc:_desc};
        const copyDocs = [...docs];
